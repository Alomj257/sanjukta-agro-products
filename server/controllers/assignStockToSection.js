...
exports.updateStock = async (req, res,next) => {
  try {
    const { sectionId, stockId } = req.params; 
    ...
    const stock = section.stocks.find((s) => s._id.toString() === stockId);
    ...
    if(stck.totalStock<qty) throw new Error("This much quantity is not available");
    stock.qty = qty;
...
exports.deleteStock = async (req, res,next) => {
  try {
    ...
    section.stocks = section.stocks.filter((s) => s._id.toString() !== stockId);
    await section.save();
...
exports.getStocks = async (req, res,next) => {
  try {
    ...
    res.status(200).json({ message: "Stocks retrieved successfully", stocks: section.stocks });
  } catch (error) {
